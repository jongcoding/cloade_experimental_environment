...
## 작업 모드: 멀티에이전트 vs 싱글에이전트

### 멀티에이전트 모드 (권장)

`/orchestrate`로 시작. 오케스트레이터가 전문 서브에이전트를 스폰해서 병렬/순차로 작업한다.

```
Orchestrator (오케스트레이터 — 지휘자)
├── Terraform Engineer   — TF 작성/apply/destroy
├── Attack Executor      — 공격 실행/검증 스크립트 생성  
├── Chain Architect      — 체인 설계 + 신규성 검증
├── Recon Scout          — AWS 서비스 탐색/기법 발견
└── Failure Analyst      — 실패 근본 원인 분석
```

에이전트 프롬프트: `.claude/agents/` 디렉토리
오케스트레이터 스킬: `.claude/skills/orchestrate/SKILL.md`

**장점**: 작업 병렬화, 역할 분리로 컨텍스트 효율적 사용, 실패 시 해당 에이전트만 재시도

### 싱글에이전트 모드

간단한 단일 Stage 작업이나 빠른 실험 시에는 기존 스킬을 직접 사용 가능.

| 명령어 | 용도 | 멀티에이전트 대응 |
|--------|------|-----------------|
| `/orchestrate` | **멀티에이전트 핵심 작업 세션** | - |
| `/harness-loop` | 싱글에이전트 Stage 작업 (단순 작업용) | Orchestrator가 대체 |
| `/chain-design` | 공격 체인 초안 설계 (30분 이내) | Chain Architect 에이전트 |
| `/attack-discovery` | 개별 기법의 실현 가능성 실험 | Recon Scout 에이전트 |
| `/novel-discovery` | 기존에 없는 새 공격 벡터 탐색 | Recon Scout 에이전트 |
| `/failure-analysis` | 검증 실패 원인 심화 분석 | Failure Analyst 에이전트 |
| `/final-packaging` | 3회 회귀 통과 후 문서/힌트 작성 | (변경 없음) |
